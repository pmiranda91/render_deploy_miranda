...
app.layout = html.Div(children=[
                    ...
                    dbc.Row([
                        dbc.Col([
                            dbc.Card([
                                html.H2("ASIMOV", style={"font-family":"Agency FB", "font-size":"50px","margin-left":"25px"}),
                                html.Hr(),

                                html.H5("Cidades:"),
                                dcc.Checklist(df_data["City"].value_counts().index,
                                df_data["City"].value_counts().index, id="check_city",
                                inputStyle={"margin-right":"10px","margin-right":"10px"}),

                                html.H5("Variável de análise:", style={"margin-top":"15px"}),
...
def render_page_content(cities, main_variable):
    ...
    df_dayly_income = df_filtered.groupby("Date")[[main_variable]].apply(operation).reset_index()
    ...
    fig_dayly_income = px.bar(df_dayly_income, y=main_variable, x="Date", orientation="v")
    fig_product_income = px.bar(df_product_income, x=main_variable, y="Product line", color="City", orientation="h")

    for fig in [fig_city, fig_payment, fig_gender, fig_dayly_income]:
        fig.update_layout(margin=dict(l=0, r=20, t=20, b=20), height=200, template="lux")
    fig_product_income.update_layout(margin=dict(l=0, r=0, t=20, b=20), height=500)
    
    return fig_city, fig_gender, fig_payment, fig_dayly_income, fig_product_income
